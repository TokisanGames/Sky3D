...
# sky_3d.current_time assumed to be in hours (0.0 to 24.0)
var current_day: int = 0
var last_checked_day: int = -1
var last_time: float = 0.0

# A IMPLEMENTAR: Sistema de planetas 
...
@onready var move_timer: Timer = $MoveTimer if has_node("MoveTimer") else null
@onready var cloud_timer: Timer = $CloudsTimer if has_node("CloudsTimer") else null

var current_weather_active: WeatherType = WeatherType.CLEAR
var weather_transition_in_progress: bool = false

const windy_shader: Shader = preload("res://addons/sky_3d/shaders/perlin_fog.gdshader")
...
var previous_state: String = "day"


var day_colors = {
    "_atm_day_tint": Color(0.8, 0.6, 0.4, 1.0),
    "_atm_horizon_light_tint": Color(1.0, 0.7, 0.4, 1.0),
    "_atm_night_tint": Color(0.4, 0.3, 0.2, 1.0)
}

var night_colors = {
    "_atm_day_tint": Color(0.4, 0.3, 0.2, 1.0),
    "_atm_horizon_light_tint": Color(0.6, 0.4, 0.2, 1.0),
    "_atm_night_tint": Color(0.3, 0.25, 0.2, 1.0)
}

var transition_duration: float = 20.0

# MOCK DATA
var mock_climate_data = {
    "today": {
        "weather_type": 0,
        "intensity": 0.5,
        "start_time": 10.0,
...
## RAIN STUFF
@onready var rain_particle: GPUParticles3D #= $Rain/RainParticles
@onready var ripple_particle: GPUParticles3D #= $Rain/RippleParticles
@onready var raining_sound: AudioStreamPlayer3D #= $Rain/RainSound
...
## SNOW STUFF
@onready var snow_particle: GPUParticles3D #= $Snow/SnowParticles
@onready var snow_ripple_particle: GPUParticles3D #= $Snow/SnowGround
...
## CLOUD STUFF
@onready var clouds_options = sky_3d.get_node("Skydome") if sky_3d else null
...
@onready var windy_volume: FogVolume #= $Sandstorm/FogVolume if has_node("Sandstorm/FogVolume") else null
...
## DEBRIS STUFF
@onready var debris_particle: GPUParticles3D = $Debris/DebrisParticles if has_node("Debris/DebrisParticles") else null
...
func _setup_rain() -> Node3D:
    ...
    var rain_particle = GPUParticles3D.new()
...
#
#
#func effect_sandstorm(quantity: float) -> void:
    #pass
#
### Weather Forecast
